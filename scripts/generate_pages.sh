#!/usr/bin/env bash
# generate_pages.sh
# Generates docs/index.html — GitHub Pages site listing all problems.
...
# ── Same metadata as generate_readme.sh ──────────────────────────────────────
declare -A META
...
# Build unique tag filter buttons
TAG_BUTTONS=""
declare -A SEEN_TAGS
for pkg in "${PACKAGES[@]}"; do
  num=$(echo "$pkg" | cut -d_ -f2)
  if [[ -n "${META[$num]+_}" ]]; then
    tag=$(echo "${META[$num]}" | cut -d'|' -f3)
  else
    tag="Uncategorized"
  fi
  if [[ -z "${SEEN_TAGS[$tag]+_}" ]]; then
    SEEN_TAGS[$tag]=1
    TAG_BUTTONS+="<button class=\"tag-btn\" data-tag=\"${tag}\" onclick=\"filterTag(this)\">${tag}</button>\n"
  fi
done
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    input[type=search] { width: 100%; background: #161b22; border: 1px solid #30363d; border-radius: 8px;
      color: #e6edf3; padding: 10px 16px; font-size: 0.95rem; outline: none; margin-bottom: 16px; }
    ...
    .diff-btn { cursor: pointer; border: 2px solid #30363d; border-radius: 20px; padding: 5px 16px;
    ...
    .tag-btn { cursor: pointer; border: 1.5px solid #58a6ff; border-radius: 20px; padding: 4px 14px;
      font-size: 0.78rem; font-weight: 600; background: transparent; color: #58a6ff; transition: all .2s; }
    .tag-btn.active { background: #58a6ff; color: #0d1117; }
    .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 16px;
      max-width: 1200px; margin: 24px auto 0; padding: 0 24px 48px; }
    ...
    .problem-title { display: block; font-weight: 600; font-size: 1rem; color: #e6edf3;
    ...
    .tag-pill { display: inline-block; font-size: 0.72rem; font-weight: 600; padding: 3px 10px;
      border-radius: 20px; margin-bottom: 12px; background: #58a6ff22; color: #58a6ff; }
...
<script>
  ...
  function applyFilters() {
    ...
    document.querySelectorAll('.card').forEach(card => {
      ...
      const matchTag    = activeTags.size === 0 || activeTags.has(card.dataset.tag);
      const matchDiff   = activeDiffs.size === 0 || activeDiffs.has(card.dataset.difficulty);
